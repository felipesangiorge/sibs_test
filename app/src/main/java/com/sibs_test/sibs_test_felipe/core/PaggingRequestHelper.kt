...
import com.sibs_test.sibs_test_felipe.core.PagingRequestHelper.*
import com.sibs_test.sibs_test_felipe.core.PagingRequestHelper.Request.Callback
import java.util.*
 ...
 */(private val mRetryService: Executor) {
    ...
    fun runIfNotRunning(type: RequestType, request: Request): Boolean {
        ...
        synchronized(mLock) {
            val hasListeners: Boolean = !mListeners.isEmpty()
    ...
    @AnyThread
    @VisibleForTesting
    fun recordResult(wrapper: RequestWrapper, throwable: Resource.Error?) {
        var report: StatusReport? = null
    ...
    fun interface Request {
        ...
        class Callback internal constructor(private val mWrapper: RequestWrapper, private val mHelper: PagingRequestHelper) {
             ...
             */
            fun recordFailure(throwable: Resource.Error) {
                if (throwable == null) {
    ...
    class StatusReport internal constructor(
         ...
         */
        val after: Status,
        private val mErrors: Array<Resource.Error?>
    ) {
        /**
         ...
         * @return The [Resource.Error] returned by the failing request with the given type or
         * `null` if the request for the given type did not fail.
         */
        fun getErrorFor(type: RequestType): Resource.Error? {
    ...
    internal inner class RequestQueue(val mRequestType: RequestType) {
        ...
        var mLastError: Resource.Error? = null
